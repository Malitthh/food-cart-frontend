...
;(function ($) {
    ...
    let $clone_to_mobile_sidebar 	= $('.clone-to-mobile-sidebar'),
    	...
    	BIOLIFE_MOBILE_MENU 		= {
    		...
    		clone_menus: function (obj) {
		            ...
		            if ($(target_id).length) {
		                ...
		                if ( typeof item_title !== 'undefined') {
		                    if (current_panel_title.length) {
		                        ...
		                        $biolife_panels.prepend('<span className="biolife-current-panel-title">'+item_title+'</span>');
		                    }
		                }else {
		                    current_panel_title.remove();
		                }
		                $biolife_panels.find('.biolife-prev-panel').remove();
		                $biolife_panels.prepend('<a data-prenttitle="'+firstItemTitle+'" className="biolife-prev-panel" href="#' + thisPanel.attr('id') + '" data-cur-panel="' + target_id + '" data-target="#' + thisPanel.attr('id') + '"></a>');
		            }
		        });
		        ...
		        $document.on('click', '.biolife-prev-panel', function (e) {
		            ...
		            if ( typeof new_parent_panel_id == 'undefined') {
		                ...
		                if ( typeof item_title !== 'undefined') {
		                    if ( !current_panel_title.length) {
		                        $('.biolife-panels-actions-wrap').prepend('<span className="biolife-current-panel-title">'+item_title+'</span>');
		                    }else{
		                        current_panel_title.html(item_title);
		                    }
		                }else {
    		...
    		insert_children: function (obj, i) {
		        ...
		        if ( children.length) {
		            children.each(function () {
		                ...
		                thisChildItem.prepend('<a className="biolife-next-panel" href="#' + next_nav_target + '" data-target="#' + next_nav_target + '"></a>');
		                ...
		                $('#biolife-clone-wrap .biolife-panels').append('<div id="' + next_nav_target + '" className="biolife-panel biolife-sub-panel biolife-hidden">' + submenu_html + '</div>');
		            });
		        }
		    },
		    ...
		    wrap_container: function(){
				...
				let mb_menu	='<div id="biolife-clone-wrap" className="biolife-clone-wrap">';
				mb_menu +='<div className="biolife-panels-actions-wrap" data-root_title="Main Panel">';
				mb_menu +='<span className="biolife-current-panel-title">Main Panel</span>';
				mb_menu +='<a className="biolife-close-btn biolife-close-panels" href="#" data-object="open-mobile-menu">&times;</a>';
				mb_menu +='</div>';
				mb_menu +='<div className="biolife-panels">';
				mb_menu +='<div id="biolife-main-panel" className="biolife-panel biolife-main-panel">';
				mb_menu +='<ul className="depth-01"></ul>';
				mb_menu +='</div></div></div>';
    ...
    $.fn.biolife_init_carousel = function () {
        ...
        $(this).not('.slick-initialized').each(function () {
            ...
            if (config.vertical === true ) {
                config.prevArrow = '<span className="biolife-icon icon-left-arrow prev"></span>';
                config.nextArrow = '<span className="biolife-icon icon-arrow-right next"></span>';
            } else {
                config.prevArrow = '<span className="biolife-icon icon-left-arrow prev"></span>';
                config.nextArrow = '<span className="biolife-icon icon-arrow-right next"></span>';
            }
    ...
    $.fn.biolife_countdown = function () {
        ...
        string_format += '<span className="days"><span className="number">%-D</span><span className="text">days</span></span>';
        string_format += '<span className="hours"><span className="number">%-H</span><span className="text">Hours</span></span>';
        string_format += '<span className="mins"><span className="number">%-M</span><span className="text">Mins</span></span>';
        string_format += '<span className="secs"><span className="number">%-S</span><span className="text">Secs</span></span>';

        html_result += '<span className="days"><span className="number">00</span><span className="text">days</span></span>';
        html_result += '<span className="hours"><span className="number">00</span><span className="text">Hours</span></span>';
        html_result += '<span className="mins"><span className="number">00</span><span className="text">Mins</span></span>';
        html_result += '<span className="secs"><span className="number">00</span><span className="text">Secs</span></span>';
    ...
    $.fn.biolife_sidebar_handle = function(){
        if( $clone_to_mobile_sidebar.length){
        	if( $sidebar.length ){
	            ...
	            html_sidebar +='<div id="sidebar" className="sidebar mobile-version">';
	            html_sidebar +='<div className="biolife-mobile-panels">';
	            html_sidebar +='<span className="biolife-current-panel-title">Sidebar</span>';
	            html_sidebar +='<a className="biolife-close-btn" href="#" data-object="open-mobile-filter">&times;</a>';
	            html_sidebar +='</div>';
	            html_sidebar +='<div className="sidebar-contain"></div>';
	            html_sidebar +='</div>';
